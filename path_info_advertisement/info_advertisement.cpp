...
int main(int argc, char* argv[])
{
   ...
   //(1)查看MySQL有几个节点，节点的数量为node_num 
   int node_num=0;
   stmt.bindout(1,&node_num);
   ...
   {  
      ...
      sprintf(strFileName,"update routingnode set trust_value=:1 where id=:2");  
      
      stmt.prepare(strFileName);
    ...
    for(int i=0;i<arc_num;i++)
   {
      ...
      stmt.bindout(1,temp_info.src,29);
      stmt.bindout(2,temp_info.dest,29);
      ...
      //开始生成三个参数
      temp_info.delay=rand()%10+1;
      temp_info.packet_loss=rand()%10+1;
      temp_info.throughput=rand()%10+1;     
      
      //第四个参数应当从routingarc去计算 
      int trust_src;
      int trust_dest;
   
      sprintf(sentence, "select trust_value from routingnode where name='%s'",temp_info.src);
      stmt.prepare(sentence);


      stmt.bindout(1,&trust_src);
     ...
     if(stmt.next()!=0)
      ...
      sprintf(sentence, "select trust_value from routingnode where name='%s'",temp_info.dest);
      stmt.prepare(sentence);

      stmt.bindout(1,&trust_dest);
      ...
      temp_info.trust_value=(trust_src+trust_dest)/2;
