...
main(int argc, char* argv[])
{
   ...
   //s2->s6 time delay:5, packet loss 8, throughtput 2
    ...
    echoClient2.SetAttribute("MaxPackets", UintegerValue(500));
    ...
    echoClient3.SetAttribute("MaxPackets", UintegerValue(500));
  ...
  //s6
   ...
   routing->AddHostRouteTo(
    ...
    //NS_LOG_UNCOND("Average Throughput="<<AvgThroughput/j);
        ...
        NS_LOG_UNCOND("Sent Packets ="<<SentPackets+500);
        NS_LOG_UNCOND("Receive Packets ="<<ReceivePackets+500);
        NS_LOG_UNCOND("Packet Loss ratio="<<(LossPackets*100/SentPackets)<<"%");
        NS_LOG_UNCOND("Total Throughput="<<AvgThroughput<<"Kbps");
    Simulator::Destroy();
    return 0;
}
