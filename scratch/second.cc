...
void EnqueueTrace(std::string context,Ptr<const Packet> pkt, Ptr<Ipv4> ip, uint32_t number)
{
  std::cout<<context<<std::endl;
  ns3::Ipv4Header ipv4;
  pkt->PeekHeader(ipv4);
       // std::cout << "User_id Ip:" <<ipv4.GetUserID()<<std::endl;
...
main (int argc, char *argv[])
{
  ...
  //Config::Connect("/NodeList/*/Internet/*/MacRx", MakeCallback(&MacRxTrace));
  
  Config::Connect("/NodeList/*/$ns3::Ipv4L3Protocol/Rx", MakeCallback(&EnqueueTrace));
