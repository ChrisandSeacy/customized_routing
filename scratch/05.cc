...
{
    bool verbose = true;
    uint32_t nCsma = 43;   //nCsma node number
   ...
   //s1->s2 time delay:10, packet loss 8, throughtput 8
    ...
    pointToPoint5.SetDeviceAttribute("DataRate", StringValue("180Mbps"));
   ...
   //s2->s6 time delay:5, packet loss 8, throughtput 2
    ...
    p2pInterfaces19= address.Assign (staDevices2);
    ...
    echoClient.SetAttribute("MaxPackets", UintegerValue(2400));
    ...
    echoClient2.SetAttribute("MaxPackets", UintegerValue(2400));
    ...
    echoClient3.SetAttribute("MaxPackets", UintegerValue(2000));
    echoClient3.SetAttribute("Interval", TimeValue(MicroSeconds(100)));
    echoClient3.SetAttribute("PacketSize", UintegerValue(1024));

    ApplicationContainer clientApps3 = echoClient3.Install(wifiStaNodes1.Get(2));
    ...
    echoClient4.SetAttribute("MaxPackets", UintegerValue(2400));
   ...
   UdpEchoServerHelper echoServer5(9);
    ...
    echoClient5.SetAttribute("MaxPackets", UintegerValue(2000));
   ...
   // Config::Connect("/NodeList/*/DeviceList/*/TxQueue/Dequeue", MakeCallback(&DequeueTrace));
        ...
        iter++)
    {
        ...
        SumThroughput=SumThroughput + (iter->second.rxBytes *8.0/(iter->second.timeLastRxPacket.GetSeconds()-iter->second.timeFirstTxPacket.GetSeconds())/1024);
    ...
    //NS_LOG_UNCOND("Average Throughput="<<AvgThroughput/j);
        ...
        NS_LOG_UNCOND("------------------------------------");
        NS_LOG_UNCOND("Delay ="<<Delay/j);
        NS_LOG_UNCOND("Sent Packets ="<<SentPackets);
         NS_LOG_UNCOND("Receive Packets ="<<ReceivePackets);
        NS_LOG_UNCOND("Packet Loss ratio="<<(LossPackets*100/SentPackets)<<"%");
